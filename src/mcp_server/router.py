...
inbox: Dict[str, List[Dict[str, Any]]] = {"AppA": [], "AppB": [], "AppC": []}
...
# Add a route to get the current inbox state (for debugging)
@router.get("/inbox_status")
async def get_inbox_status():
    """Get the current inbox status for all apps."""
    status = {}
    for app in VALID_APPS:
        status[app] = {
            "count": len(inbox.get(app, [])),
            "messages": inbox.get(app, [])
        }
    return status
